...
function getServices($dbconn){
  $result = [];
  $show = 'show';
  $stmt= $dbconn->prepare("SELECT * FROM services WHERE visibility = :show");
...
function getProjects($dbconn,$start, $record){
  $result = [];
  $stmt = $dbconn->prepare("SELECT * FROM project ORDER BY id DESC  LIMIT $start, $record");
  $stmt->execute();
  while($row = $stmt->fetch(PDO::FETCH_BOTH)){
      $result [] = $row;
    }
...
function getAllEvents($dbconn,$start, $record){
  ...
  function getOneEvent($dbconn, $hid){
    $stmt = $dbconn->prepare("SELECT * FROM events WHERE hash_id = :hid");
    $stmt->bindParam(':hid', $hid);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_BOTH);
    return $row;
...
try {
  ...
  $stmt = $dbconn->prepare("SELECT * FROM project WHERE hash_id = :hid");
  $stmt->bindParam(':hid', $hid);
  $stmt->execute();
  $row = $stmt->fetch(PDO::FETCH_BOTH);
